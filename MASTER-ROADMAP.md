...
| Tag | Task | Size | Touches | Acceptance |
|---|---|---|---|---|
| **ART-01** `[ ]` | **Muzzle + tracers.** Per-weapon flash sprite (rifle star, sniper lance, cannon bloom), tracer width by weapon, shell-eject sparks. | S | `22-effects`, `29-shooting`, `20-player` | Screenshot per operative firing. |
...
| **ART-03** `[ ]` | **Damage numbers.** Floating numbers on hits (white / gold crit / red head), capped to 24 on screen, pooled canvas sprites. | S | `22-effects`, `29-shooting` | Numbers visible; no allocations per hit. |
