...
Current build: **v3.0.0** (10 Sep 2026) — v3.0 "Meltdown" pass in progress (see the V3 table below). v2.8.0 (8 Sep 2026): Quaternius cast, UAL jump + reload, free look + RMB aim, kit props merged into the world, explosive barrels.
...
| **V3-P5** `[x]` v3.0.0-p5 | **Evolutions.** Wardens drop a Forge Core; a Forge card (also the Fabricator's 4th option) fuses any item held at 5 stacks into its evolution. Ten evolutions hooked in `tryFire` / `dealDamage` / `killEnemy` / `update`: Adrenal Cascade, Ablative Weave, Kinetic Drive, Fracture Optic, Railcore, Belt Feed, Hot Swap, Nanite Bloom, Overcharge Cell, Hemolattice. Ascension card on entering stage 3 (two mutations per operative: Siege Mode / Warcry, Ghost Step / Executioner, Bastion / Riot Charge), kept for the run. Codex lists all of it. Gate suite `test10.js` fires every one. | done |
| **V3-P6** `[x]` v3.0.0-p6 | **Meltdown Protocol.** New mode from the menu, the lobby panel and a third lobby portal: no waves; a Director accrues `8 + 1.6 × minutes` credits/s and spends on Rusher 12 / Lancer 20 / Elite 55 / Warden 240 under the alive cap (credits bank while the field is clear); reactor charge 0→100 % over 11 min on a top bar; Wardens at 25 / 55 / 85 % each drop a Core Shard — bank at the Shard Terminal (score locked, +1 item at extraction) or hold (+20 % drops, +12 % enemy damage each, cap 5); at 100 % an extraction pad opens for 90 s while the Director dumps its budget; reaching it is the first win screen. Score = banked × 1000 + held × 2500 (extraction only) + kills + time bonus; best per operative persisted; shareable score code (base32 + checksum, shared encoder for GP-10). Elite enemies (3× HP, gold). Six Trials (60–90 s, fixed maps / operatives, no items) award persisted Trial Marks. Gate `test11.js`. | done |
| **V3-P7** `[x]` v3.0.0 | **Roster.** Ranger's Mark (headshots mark 4 s, +25 % from every source) and Bulwark's Braced (−25 % while grounded and firing); Sable (SMG + Snare Charge: pull + 60 % slow), Ember (flame projector, 12 m falloff, Burn 6 dps / 3 s + Backdraft cone detonation for 40 % of remaining), Arclight (semi-auto carbine + 20 s sentry drone at 14 dps, `enemy_drone` tinted blue); unlock ladder (Vanguard always · Ranger wave 5 · Bulwark wave 10 · Sable stage 3 cleared · Ember three Wardens in a run · Arclight a Meltdown extraction, or Trial Marks), locked pods silhouetted with their requirement; the three new operatives reuse the packed human rigs with runtime palettes (no repack, bundle 7.1 MB). `FERROUS-ARENA-REVIEW.md` §5 does not exist in the repo, so only the stats the ±15 % DPS gate forced were touched (Vanguard dmg 17→14, Ember 4→6.5, Arclight 22→28). Gate `test12.js`. | done |
...
| **GP-07** `[x]` v3.0.0-p5 (as the V3-P5 evolutions; the three named procs replaced by the ten specced evolutions) | **Proc items.** Chain Lightning (hit arcs 40 % dmg to 2 enemies within 6 m), Detonator (kills explode, 30 % of max HP in 3 m), Bleed (5 %/s for 3 s). Hooks in `dealDamage` / `killEnemy`; codex entries. | M | `13-data-items`, `26-items`, `29-shooting`, `35-screens`, `test` | Each proc fires once in the harness; codex lists them. |
...
| **GP-10** `[~]` v3.0.0-p6 (score code encoder/decoder + per-operative best shipped in V3-P6; the Daily seed button and leaderboard are open) | **Daily run + local leaderboard.** Seed = date; "Daily" button on the menu; best 10 runs per seed in localStorage with a shareable score code. | S | `35-screens`, `33-mode-transitions`, `15-persistence`, `test` | Daily seed replays the same map order; score code round-trips. |
| **GP-11** `[x]` v3.0.0 (unlock ladder + locked pods in V3-P7; run history screen open) | **Meta-progression.** Ranger unlocks at best wave 5, Bulwark at 10; per-operative challenge chips on the pod; run history screen. | M | `12-data-characters`, `27-characters`, `35-screens`, `15-persistence`, `test` | Locked pods show the requirement; unlock persists. |
