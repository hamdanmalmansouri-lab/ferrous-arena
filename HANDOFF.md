...
**Status:** v3.0.0-p6 — the v3.0 "Meltdown" pass is in progress (phases 1–6 shipped: correctness, roguelite loop, art foundation, game feel, evolutions, Meltdown Protocol + Trials; phases 2–7 in `MASTER-ROADMAP.md` V3 table). Base: playable v2.8 (roadmap phases 1, 2a/2b, 4, 3, 5 — performance, touch/gamepad, PWA, pathfinding + stages,
...
| `30-abilities`, `31-damage` | `useAbility` (charge stock `player.abStock` ≤ `run.stats.abStock`; ascension effects: Ghost Step, Bastion, Riot Charge stun via `e.stun`; Warcry flag), `shakeCam(amp,dist)`, `hurtPlayer` (Warcry −30 %, shield, i-frames, haptic, flinch + shake) |
