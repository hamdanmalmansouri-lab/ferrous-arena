...
**Status:** v3.0.0 — the v3.0 "Meltdown" pass is complete (phases 1–7: correctness, roguelite loop, art foundation, game feel, evolutions, Meltdown Protocol + Trials, roster; phases 2–7 in `MASTER-ROADMAP.md` V3 table). Base: playable v2.8 (roadmap phases 1, 2a/2b, 4, 3, 5 — performance, touch/gamepad, PWA, pathfinding + stages,
...
| Bulwark | Heavy | 170 | 5.2 | Scatter cannon, 8×10 dmg, 6 shells, 2× head; **Braced** −25 % while grounded and firing | **Barrier** — shield worth 45 % of max HP for 5s (15s cd) |
| Sable | Saboteur | 95 | 6.8 | SMG, 9 dmg, 40 mag, 2.2× head | **Snare Charge** — puck, arms 0.4 s, pulls within 5 m + 60 % slow 2.5 s (8s cd) |
| Ember | Incendiary | 120 | 5.9 | Flame projector, 6.5 dmg × 22/s, 90 mag, 12 m falloff, Burn 6 dps / 3 s | **Backdraft** — ignites a cone, detonates burns for 40 % of remaining (14s cd) |
| Arclight | Technician | 100 | 6.4 | Semi-auto carbine, 28 dmg, 18 mag, 2.4× head | **Deploy Sentry** — blue drone, 20 s, 14 dps at the nearest target (20s cd) |

Unlocks (`CHARS[i].unlock`, `unlocked(ch)`): Vanguard always · Ranger best wave 5 · Bulwark best wave 10 · Sable best stage 4 (stage 3 cleared) · Ember `fa2.wardensRun` ≥ 3 · Arclight any `fa2.meltdownBest` — or Trial Marks 1 / 2 / 3 / 4 / 6. Vanguard 14 dmg, Ranger 50 (+ **Mark**: headshots mark 4 s, +25 % from every source).
...
| `13b-data-modes` | **`MELTDOWN`** (duration, window, cards, Warden thresholds, shard rules, credit rate), **`TRIALS`**, `encodeScoreCode/decodeScoreCode`, `MODE_IDS` |
| `11-constants`, `12-data-characters`, `13-data-items` | **`EVOS`** (item id → evolution), **`FUSE_STACKS`** (5), **`ASCENSIONS`** (operative id → two mutations), tuning constants, `MELEE_TOKENS`, **`timers[]` + `after(t,fn)` + `tickTimers(dt)`** (every gameplay delay goes through these, never `setTimeout`), `CHARS[]` (abilities carry a `short` touch label), `ITEMS[]` |
...
| `20-player` | `WEAPON_FX` + `flashTex` (per-weapon muzzle sprite / tracer), `player` (incl. `kick`, `flinch`, `lead`), `run`, `computeStats()`, `GUN_MOUNT`, **`MOUNTS`** (per-operative `pos/rot/back/up`), **`applyVanguardPalette`**, `buildAvatarModel(ch)` (+ `addRim`) (glTF or `buildAvatarProcedural`; sets `gun.userData.kick`), `rebuildAvatar()` (`avatarAnim`, `avatarBones`) |
...
| `22-effects` | **pools**: `TRACER_POOL` (48 additive cylinders, width + colour per weapon), `SPARK_POOL` (240, fade by scale, count × `Q.cfg.fx`), `PROJ_POOL` (160, no lights — `glowSprite()`), **`DMG_POOL`** (24 canvas sprites, `dmgNumber(pos,value,kind)`), **`DECAL_POOL`** (32 scorch planes, `decal(point,normal)`, `worldHitNormal`), `dropPickup` |
...
| `25-hud-helpers` | `say(txt,cls)` (two-line bottom-centre feed; `item` gold / `warn` hot / `scrap`), `syncHUD` (wave + Warden-in, integrity, magazine, scrap + hostiles line, ability, boss bar, touch button), `syncCompass` (crates/items/boss/portal/Fabricator bearings), `syncItems`, `syncRange`, `setMode` |
| `26-items`, `27-characters` | **Forge** (`addCore`, `forgeEligible`, `openForge/showForge/fuse`), **ascension** (`offerAscension/showAsc/pickAsc`, fired from `nextStage` at stage 3), `giveItem(id)`, **offers** (`rollTier/rollOffer`, `queueOffer`, `showOffer/pickOffer/closeOffer`, `offerMove/offerConfirm/offerKey` for pad + keys), **scrap** (`SCRAP_VALUE`, `addScrap`), **Fabricator** (`FAB_OPTS`, `spawnFabricator`, `openFabricator/showFab/fabBuy`, `showScrapPick/scrapPick/removeItemStack`); `selectChar(i, inLobby)` |
| `28-waves` | `startWave` (boss on multiples of `BOSS_EVERY`), `spawnWarden(nw,mk)`, `spawnAt(type,wave)` (chaser / shooter / elite), `spawnOne`, `waveCleared`, `bossDefeated`, `buildStage`, `nextStage`; **Meltdown**: `newMeltdown`, `meltdownTick` (Director, Wardens, pad window), `openExtraction`, `addShard/bankShards/heldShards`, `spawnTerminal`, `meltdownScore`, `extracted`; **Trials**: `newTrial`, `trialTick`, `trialProgress`, `trialEnd`, `trialMarks` |
...
| `30-abilities`, `31-damage` | `useAbility` (Snare Charge → `state.snare`, Backdraft, Deploy Sentry → `state.sentry`, ticked in the loop before the waves block; charge stock `player.abStock` ≤ `run.stats.abStock`; ascension effects: Ghost Step, Bastion, Riot Charge stun via `e.stun`; Warcry flag), `shakeCam(amp,dist)`, `hurtPlayer` (Warcry −30 %, shield, i-frames, haptic, flinch + shake) |
...
| `33-mode-transitions` | `resetPlayerFor`, `goLobby/goRange`, `goRun(seed?, kind?, trialId?)` (`run.kind` endless / meltdown / trial; seed from arg / `?seed=` / clock → `run.order`; Meltdown picks one map from the seed and spawns the terminal; a trial forces its operative and clears items), `resumePlay()`, `enterPlay()` |
...
- Roster: `CHARS[i].weapon` (rifle / marksman / scatter / smg / flame / carbine) drives SFX, the 12 m flame range + falloff, the semi-auto press gate (`player.fireHeld`); `PALETTES` recolour the reused rigs by material name; `MOUNTS` / `WEAPON_FX` per operative; burn ticks (`e.burnT/burnTick`), `e.markT`, `e.slowT` in the enemy loop.
- Meltdown: `MELTDOWN.*` in `13b-data-modes` (11 min, 90 s window, cards, 25/55/85 %, shard bonuses, `creditRate`); the Director cadence 0.4 s (0.15 s dumping); elites in `makeEnemy(type,wave,{elite:true})`. Trials: `TRIALS[]` (time, goal, spawn rate, type mix, forced operative). Score code fields: mode / char / seed / score / detail.
- Evolutions: `evo(itemId)` / `asc(id)` / `fired(id)` in `20-player`; the hooks are labelled by name in `tryFire` (Hot Swap, Railcore, Belt Feed, Siege, Kinetic), `dealDamage` (Executioner, Hemolattice, Fracture Optic), `killEnemy` (Adrenal, Hemolattice, Forge Core), `update` (charges, Kinetic window, Siege timer, Nanite, Weave, Warcry heal, Bastion slow, stun). `state.evoFired` / `state.ascFired` count firings for the harness; `state.lastShot` holds the last bullet line.
- Effects: `WEAPON_FX[id]` (flash kind / size, tracer width / colour), hit-stop 0.06 / 0.14 s at 12 % speed in `killEnemy` + `frame()`, dissolve 0.8 s in the corpse loop, decal life 8 s, damage numbers 0.7 s. HUD tokens `--g --r --hero --body --label` in `styles.css`.
...
node test12.js  # v3 gate: unlock ladder (fresh save, thresholds, Trial Marks), 6 lobby pods with locked labels, Mark / Braced / Snare / Burn + Backdraft / Sentry / semi-auto, sustained body DPS on the range within 15% of the mean
node test11.js  # v3 gate: menu + lobby entry points, Director spends credits, Warden at 25% drops a shard (no portal), held-shard damage bonus, terminal banking, pad at 100% + extraction screen, score formula, score code round-trip, best persisted, a trial completes and persists a mark
node test10.js  # v3 gate: Forge card fuses at 5 stacks + a core; each of the 10 evolutions and 6 ascensions fires; codex lists them
node test9.js   # v3 gate: decal / damage number / hit-stop / dissolve sanity, then a 10 s wave-5 capture into Claude outputs/wave5 (+ contact sheet) with an impact frame per kill
