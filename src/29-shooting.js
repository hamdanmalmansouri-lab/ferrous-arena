...
function tryFire(){
  ...
  if(player.freeAmmoT>0)fired('hotswap'); else player.mag--;                                   // Hot Swap: free ammo after a reload
  const rail=evo('rounds')&&(++player.shotIdx%6===0); if(rail)fired('railcore');               // Railcore: every 6th shot pierces for 250%
  const belt=evo('extmag')&&player.mag<=s.mag*0.25; if(belt)fired('beltfeed');                  // Belt Feed: last quarter of the magazine
  const siege=asc('siege')&&player.siegeT>=0.5; if(siege)fired('siege');                        // Siege Mode: planted feet
  ...
  const baseSpread=(s.spread+(inp.sprint?0.01:0)+(player.vel.lengthSq()>2?0.004:0))*(1-0.4*player.aimK)*(siege?0.5:1);   // aiming (and Siege Mode) tightens the cone
  let pelletHits=0;
  for(let p=0;p<s.pellets;p++){
    ...
    const endPoint=h.point; state.lastShot={o:muzzle.toArray(),d:dir.toArray()};   // bullet line of the last pellet (harness + debugging)
    if(h.obj){
